...
class TestBaseLearnerInitialization:
    ...
    def test_learner_has_id(self, learner):
        assert learner.id is not None
        assert isinstance(learner.id, str)

    def test_learner_has_skill_space(self, learner, skill_space):
        assert learner.skill_space is not None
        assert learner.skill_space == skill_space

    def test_learner_has_mastered_skills(self, learner, selected_config):
    ...
    def test_learner_practice_history_logged_on_init(self, learner):
        assert len(learner.practice_history) == 1
        assert learner.practice_history[0]["session_id"] == "initialization"

    def test_learner_practice_file_exists(self, learner):
        assert Path(learner.practice_conversations_file).exists()

    def test_duplicate_skills_rejected(self, skill_space, tmp_path):
...
class TestBaseLearnerHasSkill:
    ...
    def test_has_mastered_skill_returns_true(self, learner, selected_config):
        for skill_id in selected_config["mastered_skills"]:
            assert learner.has_skill(skill_id) is True

    def test_has_unmastered_skill_returns_false(self, learner, skill_space):
        unmastered = [
            sk.id for sk in skill_space if sk.id not in learner.mastered_skills
        ]
        if unmastered:
            assert learner.has_skill(unmastered[0]) is False

    def test_has_nonexistent_skill_returns_false(self, learner):
...
class TestBaseLearnerKnowledge:
    ...
    def test_get_knowledge_description_returns_string(self, learner):
        desc = learner.get_knowledge_description()
        assert isinstance(desc, str)
        assert len(desc) > 0

    def test_knowledge_description_mentions_mastered_skills(self, learner):
        desc = learner.get_knowledge_description()
        for skill_id in learner.mastered_skills:
            assert skill_id in desc

    def test_empty_learner_knowledge_description(self, skill_space, tmp_path):
