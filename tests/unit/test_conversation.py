...
class TestConversationSetup:
    ...
    def test_skill_space_loads(self, skill_space):
        assert skill_space is not None
        assert len(skill_space.skills) > 0
        assert "MA.6.NSO.2.2" in skill_space

    def test_practice_item_pool_loads(self, practice_item_pool):
    ...
    def test_graph_memory_path_is_db(self, conversation):
...
class TestConversationState:
    ...
    def test_state_initial_values(self, conversation):
    ...
    def test_state_has_skill_fields(self, conversation):
        initial_state = conversation.initialize_state({})
...
class TestConversationGraphStructure:
    ...
    def test_graph_compiles(self, conversation):
        compiled_graph = conversation.compile_graph()
        assert compiled_graph is not None

    def test_graph_has_start_and_end(self, conversation):
        compiled_graph = conversation.compile_graph()
    ...
    def test_print_graph_method_exists(self, conversation):
        compiled_graph = conversation.compile_graph()
